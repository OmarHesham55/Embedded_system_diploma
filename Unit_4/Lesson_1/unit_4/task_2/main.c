 ...
 */
							...
							///////// second task-- number of student by iterative way //////
...
void records(){
	...
	if(firstStudent){
		...
		DPRINTF("\n\nNo students data exist\n");
	if(count>0){
		DPRINTF("number of records is:%d\n",count++);
	}else
		DPRINTF("no records exist\n");
}
...
int main(){
	...
	while(1){
		...
		DPRINTF("5-Records of the linked list --by iterative way--\n")
