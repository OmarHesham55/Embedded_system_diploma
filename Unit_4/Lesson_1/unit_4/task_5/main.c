...
//============================================ --reverse node from the end-- //second way =======================================
...
void reverse_Student(){
	int count=0,len,node;

	struct student* mainPnt = firstStudent;
	struct student* reversePnt = firstStudent;

	if(firstStudent){
		while(mainPnt){
			mainPnt = mainPnt->nextStudent;
			count++;
		}
		DPRINTF("Enter which node you want?\n");
		scanf("%d",&node);
		len = count;
		if(node<len){
			int temp = len - node + 1;
			count = 1;
			DPRINTF("the record of %d from the last is:\n",node);
			while(count < temp){
				reversePnt = reversePnt->nextStudent;
				count++;
			}
			DPRINTF("===================================\n");
			DPRINTF("ID: %d\n",reversePnt->student.Id);
			DPRINTF("Name: %s\n",reversePnt->student.name);
			DPRINTF("Age: %d\n",reversePnt->student.age);
			DPRINTF("height: %.1f\n",reversePnt->student.height);
			DPRINTF("gender: %s\n",reversePnt->student.gender);
			DPRINTF("===================================\n\n")
		}else
			DPRINTF("the maximum number to check is %d\n",len);
